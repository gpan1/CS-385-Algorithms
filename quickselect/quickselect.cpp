...
int main(int argc, char *argv[]) {
    ...
    iss.str(argv[1]);
    if(!(iss >> k) ||num_values < k)
    {
    	if(num_values == 1)
    	{
    		cerr << "Error: Cannot find smallest element " << argv[1] << " with only " << num_values <<" value.";
    	}
    	else
    	{
    ...
    cout << "Smallest element " << argv[1] << ": " << quick_select(&values[0], num_values, k);
	iss.clear();
    return 0;
}
